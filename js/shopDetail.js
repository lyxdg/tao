...
class buyOrAdd {
  ...
  setPageGoodId() {
    axios.get("./php/user.php", {
      ...
      // 页面标题
      document.querySelector("title").innerHTML = this.goodId + "号商品详情页面";
  ...
  initPage() {
    axios.get("./php/goods.php", {
      ...
      goodTypes.forEach((li, i) => {
        ...
        // 给商品款式绑定移入事件
        li.addEventListener("mouseenter",this.typeFn.bind(this,li,imgObj.src,smallImgObj,bigImgObj));
  ...
  /* 款式改变方法 */
  typeFn(li,imgSrc,smallImgObj,bigImgObj){
    smallImgObj.src = imgSrc;
    bigImgObj.src = imgSrc;
    document.querySelector("li.now").classList.remove("now");
  ...
  createSaleGood() {
    axios.get("./php/goods.php", {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        goodDiv.onclick = this.jumpGoodDetail.bind(this, good.goodId, good.ahref);
  ...
  altDBlData(goodId, num = 0) {
    ...
    axios.get("./php/user.php", {
      ...
      if (goodsCart) {
        ...
        // 1-4. 更新当前登录用户的购物车数据
        this.upUserGoods(JSON.stringify(goodsCart));

      } else {
        // 2. 当购物车里没有数据时
        // 将构建的对象以数组的形式转化为json字符串，更新到当前登录用户的购物车里
        this.upUserGoods(JSON.stringify([cGoods]));
      }
    });
